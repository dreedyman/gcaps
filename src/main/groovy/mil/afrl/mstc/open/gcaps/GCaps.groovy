...
class GCaps {
    ...
    private doSetAnalysisVal(String varName, int capsSType, varValue) {
        ...
        int result = cCAPS.caps_childByName(byReference,
                                            ...
                                            mObjRef)
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_childByName: ${CAPSError.valueOf(result)}"
            throw new IllegalStateException("caps_childByName for $varName, [$varValue] failed: ${CAPSError.valueOf(result)}")
        }
        ...
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_getValue: ${CAPSError.valueOf(result)}, nErr: ${nErr.value}"
            throw new IllegalStateException("caps_getValue for $varName, [$varValue] failed: ${CAPSError.valueOf(result)}")
        }
        def value = []
        ...
        if(dataPointer!=null) {
            ...
            result = CAPSError.CAPS_BADTYPE.code
        }
        result
    }
    ...
    def preAnalysis() {
        ...
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_preAnalysis: ${CAPSError.valueOf(result)}, nErr: ${nErr.value}"
        }
        result
    }
    ...
    def postAnalysis() {
        ...
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_info: ${CAPSError.valueOf(result)}"
            return result
        }
        ...
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_postAnalysis: ${CAPSError.valueOf(result)}, nErr: ${nErr.value}"
        }
        result
    }
    ...
    def getAnalysisOutVal(String varName) {
        if(varName!=null) {
            ...
            if (result != CAPSError.CAPS_SUCCESS.code) {
                println "caps_getValueShape: ${CAPSError.valueOf(result)}, nErr: ${nErr.value}"
                return null
            }
            CAPSVtype capsVType = CAPSVtype.valueOf(vType.value)
            switch (capsVType) {
    ...
    def close() {
        ...
        if(result!=CAPSError.CAPS_SUCCESS.code) {
            println "caps_close: ${CAPSError.valueOf(result)}"
        }
        result
    }
